...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        log.exception("failed to load LuxTTS engine")
        raise

    app.state.engine = engine
    log.info(
        "engine ready: model=%s device=%s dtype=%s sample_rate=%d",
        settings.luxtts_model,
        engine.device,
        engine.dtype_str,
        engine.sample_rate,
    )
...
app = FastAPI(title="lux-tts-open-tts", version="1.0.0", lifespan=lifespan)
...
def _resolve_voice(voice: str, request: Request) -> Voice:
    ...
    # LuxTTS is a clone-only engine — bare names are not accepted.
    raise HTTPException(
        status_code=422,
...
async def clone(
    ...
    if not prompt_text.strip():
        # Accept but do not enforce semantics — LuxTTS ignores this field.
        # Still require non-empty to stay consistent with spec §5.1.
    ...
    # Bounds-check engine-specific fields (Form() does not run Pydantic validators).
    if num_steps is not None and not 1 <= num_steps <= 64:
        raise HTTPException(status_code=422, detail="num_steps must be in [1, 64]")
    if guidance_scale is not None and not 0.0 <= guidance_scale <= 10.0:
        raise HTTPException(
            status_code=422, detail="guidance_scale must be in [0.0, 10.0]"
        )
    if t_shift is not None and not 0.0 <= t_shift <= 1.0:
        raise HTTPException(status_code=422, detail="t_shift must be in [0.0, 1.0]")
    if duration is not None and not 0.5 <= duration <= 30.0:
        raise HTTPException(status_code=422, detail="duration must be in [0.5, 30.0]")
    ...
    tmp_dir = Path(tempfile.gettempdir()) / "lux-tts-open-tts"
